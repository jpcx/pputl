...
decltype(countof) countof = NIFTY_DEF(countof, [&](va args) {
  ...
  if constexpr (conf::cpp20_arglimit) { // slower implementation but ensures
    ...
    n[0] = def{std::to_string(conf::size_max) + "(e, _, ...)"} = [&](arg e, arg, va) {
      return pp::va_opt(fail(e) + ", ") + " " + size_max_s;
    };

    for (std::size_t i = 1; i < n.size(); ++i) {
      n[i] = def{std::to_string(conf::size_max - i) + "(e, _, ...)"} = [&](arg e, arg,
                                                                           va  args) {
    ...
    }}(str("[" + countof + "] too many arguments : " + args), args);
  } else {
    def read = def{"read(_err, " + utl::cat(utl::alpha_base52_seq(conf::size_max), ", ")
                   + ", _sz, ...)"};
    ...
    def verifier = def{prefix + "(...)"} = [&] { return ""; };

    read = [&](pack args) {
      def res = def{prefix + "(...)"} = [&](va args) { return args; };

      def<"0(_err, _sz)"> _0 = [&](arg _err, arg) { return fail(_err); };
    ...
    }}(str("[" + countof + "] too many arguments : " + args), prefix, args);
  }
});
