#ifndef CODEGEN_API_IMPL_HEXHEX_TRAITS_CC
#define CODEGEN_API_IMPL_HEXHEX_TRAITS_CC
/* /////////////////////////////////////////////////////////////////////////////
//                          __    ___
//                         /\ \__/\_ \
//   _____   _____   __  __\ \ ,_\//\ \
...
inline def<"impl_hexhex_traits(v, t: 00|01|...|FE|FF, LT|AND|OR|XOR|SUB0|SUB1|ADD0|ADD1)">
    hexhex_traits = [](arg v, arg t) {
      category = "impl";
      ...
      def{"\\LT(" + utl::cat(utl::alpha_base52_seq(1), ", ")
          + ", ...)"} = [&](pack args) {
        return args[0];
      };
      def{"\\AND(" + utl::cat(utl::alpha_base52_seq(2), ", ")
          + ", ...)"} = [&](pack args) {
        return args[1];
      };
      def{"\\OR(" + utl::cat(utl::alpha_base52_seq(3), ", ")
          + ", ...)"} = [&](pack args) {
        return args[2];
      };
      def{"\\XOR(" + utl::cat(utl::alpha_base52_seq(4), ", ")
          + ", ...)"} = [&](pack args) {
        return args[3];
      };
      def{"\\SUB0(" + utl::cat(utl::alpha_base52_seq(6), ", ")
          + ", ...)"} = [&](pack args) {
        return args[4] + ", " + args[5];
      };
      def{"\\SUB1(" + utl::cat(utl::alpha_base52_seq(8), ", ")
          + ", ...)"} = [&](pack args) {
        return args[6] + ", " + args[7];
      };
      def{"\\ADD0(" + utl::cat(utl::alpha_base52_seq(10), ", ")
          + ", ...)"} = [&](pack args) {
        return args[8] + ", " + args[9];
      };
      def{"\\ADD1(" + utl::cat(utl::alpha_base52_seq(12), ", ")
          + ", ...)"} = [&](pack args) {
