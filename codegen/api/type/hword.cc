...
decltype(hword) hword = NIFTY_DEF(hword, [&](va args) {
  docs << "[inherits from " + some + "] a tuple of exactly " + word_size + " ("
              + std::to_string(conf::word_size) + ") nybls."
       << "constructibe from any word.";

  tests << hword(0)          = pp::tup(std::vector<std::string>(conf::word_size, "0")) >> docs;
  tests << hword(uint_max_s) = pp::tup(std::vector<std::string>(conf::word_size, "F")) >> docs;
  if constexpr (conf::word_size > 1) {
    tests << hword(int_min_s) =
        pp::tup("8, " + utl::cat(std::vector<std::string>(conf::word_size - 1, "0"), ", ")) >> docs;
    tests << hword(int_max_s) =
  ...
  }}(istr("[" + hword + "] invalid integer or word : " + args), some(args));
});
