...
TEST("pputl.rest: return all passed arguments except for the first") {
  static_assert(streq(STR(PPUTL_REST()), ""));
  static_assert(streq(STR(PPUTL_REST(a)), ""));
  static_assert(streq(STR(PPUTL_REST(a, b)), "b"));
  static_assert(streq(STR(PPUTL_REST(a, b, c)), "b, c"));
};
